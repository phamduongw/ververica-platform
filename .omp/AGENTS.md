# VVP LAB — OMP project context

Repository chứa baseline VVP 3.1.3 cho Kubernetes và OpenShift. Nội dung bên dưới được nạp tự động làm context đầu phiên; đây là tài liệu bàn giao, không phải bằng chứng về trạng thái cluster hiện tại.
...
5. Values; với Kubernetes, tạo file post-renderer tại bước này, chưa thực thi script. Script được Helm gọi khi template/install/activation qua `--post-renderer`.
...
Trong phiên đã chạy offline: heredoc cat roundtrip/byte parity; bash -n; render cả hai profile với PyYAML 6.0.3, không có .work; assertions CRD, scheduling/security, Service/PVC và runtime defaults; post-renderer thiếu/null/selector khác; stub smoke token và activation failure ordering, restart Pod. Không chạy SQL, install/upgrade, helper thật hoặc cleanup lên cluster bằng assistant.

**Cleanup chỉ được smoke bằng stub, chưa được xác minh trên cluster.** Kết quả stub không chứng minh xóa tài nguyên thành công hoặc vòng chờ sẽ kết thúc. Block hiện tại cố ý bỏ qua lỗi Helm và có vòng chờ không timeout; các giới hạn tại mục Cleanup vẫn áp dụng.
